...
const DEFAULT_BALANCE = {
    ...
    base_food_rate: 0,
    max_food: 255,
    hungry_lifespan_multiplier: 1.5,
    clone_threshold: 0.95,
    clone_lifespan: 80,
    clone_satiety_fraction: 0.5,
};
...
export class GridState {
    constructor(color = "#FFFFFF", occupied = false, cell = null, food = 0, food_rate = 0, max_food=255) {
        this.color = color;
...
export class Grid {
    constructor(columns, rows, satiety=5, lifespan=100, decay=3, num_food_sources=10, food_radius=10, max_food_rate=10, balance={}) {
        ...
        for (let i = 0; i < this.columns; i++) {
            ...
            for (let j = 0; j < this.rows; j++) {
                board[i][j] = new GridState(
                    ...
                    false,
                    null,
                    this.balance.initial_food,
                    this.balance.base_food_rate,
                    this.balance.max_food
                );
            }
        }
        return board;
    ...
    get_lifespan_decay(neighbors, is_hungry) {
        let crowding_penalty = 1 + neighbors;
        if (is_hungry) {
            crowding_penalty *= this.balance.hungry_lifespan_multiplier;
        }
        ...
        for (let i = 0; i < this.columns; i++) {
            for (let j = 0; j < this.rows; j++) {
                ...
                if (curr_grid_state.occupied) {
                    ...
                    let is_hungry = curr_cell.satiety <= 0;
                    curr_cell.lifespan -= this.get_lifespan_decay(neighbors, is_hungry);
                    ...
                    if (curr_cell.lifespan > 0 && !curr_cell.digesting) {
                        ...
                        // clone 
                            ...
                            if (curr_cell.satiety >= curr_cell.max_satiety * this.balance.clone_threshold) {
                                if (this.in_bounds(newX, newY)) {
                                    // make clone cell function?
                                    let clone = copy_cell(curr_cell);
                                    clone.clock = 0;
                                    clone.lifespan = this.balance.clone_lifespan;
                                    clone.satiety = curr_cell.satiety * this.balance.clone_satiety_fraction;
                                    curr_cell.satiety *= (1 - this.balance.clone_satiety_fraction);
                                    let potential_grid_state = this.states[newX][newY];
                                    let next_grid_state = next_board[newX][newY];
                        ...
                        // eat
                            ...
                            if (this.in_bounds(newX, newY)) {
                                let potential_grid_state = this.states[newX][newY];
                                let next_grid_state = next_board[newX][newY];
                                let target_state = potential_grid_state.occupied ? potential_grid_state : next_grid_state;
                                if ((potential_grid_state.occupied &&
                                    potential_grid_state.cell.color != curr_cell.color) ||
                                    (next_grid_state.occupied &&
                                    next_grid_state.cell.color != curr_cell.color)) {
                                    next_board[newX][newY].accept_cell(curr_cell);
                                    curr_cell.eat_food(target_state.cell.satiety * 0.75);
                                    next_board[i][j].remove_cell();
                                } 
